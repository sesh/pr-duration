...
class PR:
    ...
    def open_duration(self):
        if self._json["merged_at"]:
            return (
                datetime.fromisoformat(self._json["merged_at"])
                - datetime.fromisoformat(self._json["created_at"])
            ).total_seconds()

    def is_merged(self):
        return self._json["merged_at"] != None
...
@click.option(
    "--max-prs",
    help="The maximum number of PRs to request from Github",
    default=500
)
@click.pass_context
def cli(ctx, token, repo, max_age, excluded_authors, authors, max_prs):
    ...
    if ctx.invoked_subcommand is None:
        ...
        durations = [pr.open_duration() for pr in prs]
...
def graph(ctx):
    ...
    while start < now:
        in_range = [
            pr for pr in prs
            if pr.merged_at() < start and pr.merged_at() > start - timedelta(days=window_days)
        ]
